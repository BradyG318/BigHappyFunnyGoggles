...
# ----------------------------------------------------------------------
def save_data_to_database(encoding, image_source, id):
    """Save the vector to PostgreSQL database."""
    print(f"\n--- DATABASE SAVE: Image Source {image_source} ---")
    success = DBLink.db_link.replace_encoding(id, encoding.tolist())
    if not success:
...
def process_single_image(image_source, face_id):
    """
    ...
    # Padding
    pad_x = 0.2 * (right - left)
    pad_y = 0.2 * (bottom - top)
    ...
    # Save to DB
    return save_data_to_database(encoding, image_source, face_id)
...
def process_batch(json_file):
    ...
    Supported formats:
      1) List of lists: [[face_id, image_path], ...]
      2) List of dicts: [{"face_id": "image_path"}, ...]  (single or multiple pairs per dict)
    """
    ...
    # Flatten the list into a list of (face_id, image_path) pairs
    entries = []
    for item in data:
        if isinstance(item, list):
            ...
            if len(item) >= 2:
                ...
                print(f"Skipping invalid list item: {item}")
        elif isinstance(item, dict):
            # Format 2: dict with id->url pairs
            for id_str, url in item.items():
                try:
                    face_id = int(id_str)   # convert string key to int
                except ValueError:
                    print(f"Invalid face ID (not an integer): {id_str}, skipping")
                    continue
                entries.append((face_id, url))
        else:
            print(f"Skipping non-list/non-dict item: {item}")
    ...
    for face_id, image_path in entries:
        if process_single_image(image_path, face_id):
            success_count += 1
        else:
            fail_count += 1
